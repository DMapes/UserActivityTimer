...
while today == datetime.date.today():
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        futures_list.append(executor.submit(ms_track_time, datetime.datetime.now()))
        keyboard_ex = executor.submit(kb_track_time, datetime.datetime.now())
        futures_list.append(executor.submit(kb_track_time, datetime.datetime.now()))
        # mouse_time = executor.submit(add_time, mouse_ex.result())
        futures_list.append(executor.submit(add_time, mouse_ex.result()))
        # kb_time = executor.submit(add_time, keyboard_ex.result())
        futures_list.append(executor.submit(add_time, keyboard_ex.result()))
